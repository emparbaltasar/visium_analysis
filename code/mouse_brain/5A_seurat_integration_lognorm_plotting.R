...
for (i in rownames(visium_subset@assays$prediction.score.id@data)) {
  ...
  ggsave(paste("./output/images/mouse_brain_integration/seurat_lognorm_prediction_", i, ".png", sep = ""))
}
...
pal <- palettePaired[1:length(ct)]

names(pal) <- ct
#saveRDS(pal, "./output/processed_data/mouse_brain/spatial_scatterpie_palette.rds")
...
plot <- plotSpatialScatterpie(
  ...
  scale_y_reverse()
ggsave("./output/images/mouse_brain_integration/seurat_lognorm_prediction_spatial_scatterpie.png")
ggsave("./output/images/mouse_brain_integration/seurat_lognorm_prediction_spatial_scatterpie.pdf")
...
ggsave("./output/images/mouse_brain_integration/seurat_lognorm_prediction_filtered_spatial_scatterpie.png")
ggsave("./output/images/mouse_brain_integration/seurat_lognorm_prediction_spatial_filtered_scatterpie.pdf")
...
proportions_comparison %>%
  ...
  stat_cor(method = "pearson", label.x = 0.08, label.y = 0.15, size = 3) +
  stat_cor(method = "spearman", label.x = 0.08, label.y = 0.14, cor.coef.name = "rho", size = 3)


ggsave("./output/images/mouse_brain_integration/seurat_lognorm_comparison_celltype_proportions_scatter.png", width = 5, height = 4)
