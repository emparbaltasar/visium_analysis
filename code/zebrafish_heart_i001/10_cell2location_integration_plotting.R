...
cell2loc_pred <- read.csv("./output/cell2location/zebrafish_heart/SCD-VI-i001/cell2location_map/cell2location_prediction.csv",
                          row.names=1)
...
for (i in rownames(visium_subset@assays$cell2loc@data)) {
  ...
  ggsave(paste("./output/images/SCD-VI-i001_integration/cell2location_prediction_abundance_", i, ".png", sep = ""))
}

SpatialFeaturePlot(visium_subset, features = "Macrophage (apoeb)", alpha = c(0,3), pt.size.factor = 2, crop = TRUE) +
  ggplot2::scale_fill_gradientn(colours = MyPalette)
ggsave("./output/images/SCD-VI-i001_integration/cell2location_prediction_abundance_macrophage(apoeb)_alpha.png", height = 5)
...
visium_subset[["cell2loc_prop"]] <- CreateAssayObject(t(cell2loc_pred_prop))

# Plot spatial scatterpie
pal <- readRDS("./output/processed_data/SCD-VI-i004/spatial_scatterpie_palette.rds")

DefaultAssay(visium_subset) <- "cell2loc_prop"
...
ggsave("./output/images/SCD-VI-i001_integration/cell2location_prediction_spatial_scatterpie.png")
ggsave("./output/images/SCD-VI-i001_integration/cell2location_prediction_spatial_scatterpie.pdf")

# Plot cell type correlation heatmap:
plotCorrelationMatrix(as.matrix(cell2loc_pred_prop),
                      insig = "pch",
                      pch.cex = 2)
ggsave("./output/images/SCD-VI-i001_integration/cell2location_correlation_matrix_proportions.png", width = 12, height = 12)
plotCorrelationMatrix(as.matrix(cell2loc_pred),
                      insig = "pch",
                      pch.cex = 2)
ggsave("./output/images/SCD-VI-i001_integration/cell2location_correlation_matrix_abundance.png", width = 12, height = 12)
...
ggsave("./output/images/SCD-VI-i001_integration/cell2location_colocalization_heatmap.png", width = 12, height = 12)
...
ggcorrplot(
  ...
  theme(
    ...
    axis.text = element_text(vjust = 0.5),
    legend.position = "top")
ggsave("./output/images/SCD-VI-i001_integration/cell2location_colocalization_heatmap_significant.png", width = 7, height = 7)


# Plot proportion per cell type

for (i in rownames(visium_subset@assays$cell2loc_prop@data)) {
  p <- SpatialFeaturePlot(visium_subset, features = i, pt.size.factor = 2, crop = TRUE) +
  ...
  ggsave(paste("./output/images/SCD-VI-i001_integration/cell2location_prediction_proportion_", i, ".png", sep = ""))
}
...
ggsave("./output/images/SCD-VI-i001_integration/cell2location_prediction_proportion_adjusted_fibroblast_(mpeg1.1).png")

SpatialFeaturePlot(visium_subset, features = "Fibroblast (col12a1a)", pt.size.factor = 2, crop = TRUE) +
  ggplot2::scale_fill_gradientn(colours = NewPalette, limits = c(0, 0.54), breaks = c(0.0, 0.25, 0.5))
ggsave("./output/images/SCD-VI-i001_integration/cell2location_prediction_proportion_adjusted_fibroblast_(col12a1a).png")

SpatialFeaturePlot(visium_subset, features = "Fibroblast (cxcl12a)", pt.size.factor = 2, crop = TRUE) +
  ggplot2::scale_fill_gradientn(colours = NewPalette, limits = c(0, 0.5), breaks = c(0.0, 0.25, 0.5))
ggsave("./output/images/SCD-VI-i001_integration/cell2location_prediction_proportion_adjusted_fibroblast_(cxcl12a).png")
...
cell2loc_prediction <- visium_subset@assays[["cell2loc_prop"]]@data %>% t()
...
ggsave("./output/images/SCD-VI-i001_integration/cell2location_comparison_celltype_proportions_scatter.png", width = 10, height = 6)
